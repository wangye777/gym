...
class ObjectTransitionV4Env(gym.Env):
    metadata = {
    ...
    def __init__(self):
        ...
        self.low_state = np.array([self.region[0], -self.max_vel, self.region[2], -self.max_vel, self.goal_reg[0], self.goal_reg[2], self.goal_rad])
        ...
        self.high_state = np.array([self.region[1], self.max_vel, self.region[3], self.max_vel, self.goal_reg[1], self.goal_reg[3], self.goal_rad])
        for i in xrange(self.num_obsts):
    ...
    def _reset(self):
        ...
        if _distance([self.state[0],self.state[2]], [self.goal[0],self.goal[1]]) < 30: self._reset()
    ...
    def _step(self, raw_action):
        ...
        self.state[0:4] = np.array([position[0], velocity[0], position[1], velocity[1]])
